...
public class Julia extends Set {
	...
	public int fractalSet(double real, double img, int iterations) {
		...
		for (times = 0; times < iterations; times++) {
			...
			double new_real = (real * real) - (img * img) + consX;
			// imaginary part when two complex numbers are multiplied
			double new_img = (2 * img * real) + consY;

			real = new_real;
			img = new_img;

			// chhecking whether the absolute value of the complex number is less than 2
	...
	public void run() {
		try {
			...
			System.out.println("Error while running Julia: " + err);
		}
	}
